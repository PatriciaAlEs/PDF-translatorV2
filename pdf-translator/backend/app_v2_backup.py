...
def extract_images_from_pdf(pdf_path, output_dir):
    """Extrae imágenes del PDF usando PyMuPDF. Más robusto con formatos variados."""
    import fitz
    images_info = []
    try:
        ...
        for page_num in range(len(doc)):
            ...
            for img_idx, img_ref in enumerate(img_list):
                try:
                    ...
                    img_filename = f"img_p{page_num+1}_{img_idx}.{img_ext}"
                    img_path = output_dir / img_filename
                    with open(str(img_path), "wb") as img_f:
                    ...
                    print(f"  Imagen omitida p{page_num+1}/{img_idx}: {e}")
        doc.close()
    except Exception as e:
...
# ─────────────────────────────────────────────
# 3. EXTRAER TEXTO (PyMuPDF — alta calidad)
# ─────────────────────────────────────────────

def _extract_text_pymupdf(pdf_path):
    """
    Extrae texto de un PDF con PyMuPDF usando reconstrucción inteligente:
      - Preserva el orden de lectura real (columnas, layout)
      - Reconstruye párrafos rotos por saltos de línea del PDF
      - Filtra encabezados/pies de página repetidos
      - Detecta y preserva diálogos, capítulos y separadores
      - Elimina guiones de separación silábica al final de línea
    """
    import fitz
    doc = fitz.open(str(pdf_path))
    all_pages_text = []
    ...
    for page_num in range(len(doc)):
        ...
        # Filtrar solo bloques de texto (type 0), ordenar por posición vertical
        ...
        # Detectar header/footer (bloques en el 8% superior/inferior)
        header_cutoff = page_h * 0.08
        footer_cutoff = page_h * 0.92
        ...
        for b in text_blocks:
            ...
            # Filtrar headers/footers repetidos (números de página, títulos corrientes)
            if y_top < header_cutoff:
                if raw == prev_header or re.match(r'^\d+$', raw):
                    continue
                prev_header = raw
            if y_bot > footer_cutoff:
                if raw == prev_footer or re.match(r'^\d+$', raw):
                    continue
                prev_footer = raw
    ...
    # Reconstruir párrafos: unir líneas que fueron cortadas por el layout del PDF
...
def _reconstruct_paragraphs(text):
    """
    Reconstruye párrafos a partir del texto crudo del PDF.
    Une líneas que pertenecen al mismo párrafo y preserva:
      - Saltos de párrafo reales (líneas vacías)
      - Diálogos (líneas que empiezan con — o comillas)
      - Títulos de capítulo
      - Separadores de escena (*** etc.)
      - Guiones de separación silábica (pala-\\nbra → palabra)
    """
    lines = text.split('\n')
    result = []
    buffer = ""
    ...
    for line in lines:
        ...
        if not stripped:
            if buffer:
                ...
                buffer = ""
            result.append("")  # preservar separación
            continue
        ...
        # Diálogo o línea que claramente empieza un nuevo párrafo
        is_new_para = (
            stripped.startswith(('—', '"', '\u201c', '\u00ab', '\u2018'))
            or (len(stripped) > 0 and stripped[0].isupper() and buffer and buffer.endswith('.'))
        )

        if is_new_para and buffer:
            result.append(buffer)
            buffer = ""

        # Unir con el buffer anterior
        if buffer:
            # Guion de separación silábica al final: pala- + bra → palabra
            if buffer.endswith('-'):
...
def extract_text():
    ...
    text = _extract_text_pymupdf(part_path)
